...
pub struct DndSource<'a, Message, AppMessage> {
    ...
    drag_content: Option<Box<dyn Fn() -> ClipboardCopy>>,
...
impl<'a, Message: 'static, AppMessage: 'static> DndSource<'a, Message, AppMessage> {
    ...
    pub fn with_drag_content(mut self, f: impl Fn() -> ClipboardCopy + 'static) -> Self {
        self.drag_content = Some(Box::new(f));
        self
    }

    pub fn with_drag_icon(
        mut self,
    ...
    for DndSource<'a, Message, AppMessage>
{
    ...
    fn on_event(
        ...
        match event {
                        ...
                        if state.hovered {
                            if let Some(left_pressed_position) = state.left_pressed_position {
                                if position.distance(left_pressed_position) > self.drag_threshold {
                                    self.start_dnd(clipboard, state.cached_bounds.clone());
            ...
            Event::Dnd(
                DndEvent::Source(SourceEvent::Cancelled) | DndEvent::Source(SourceEvent::Finished),
            ) => {
