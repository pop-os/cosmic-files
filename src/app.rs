...
impl DialogPages {
    ...
    pub fn push_back(&mut self, page: DialogPage) -> Task<Message> {
        let task = if self.pages.is_empty() {
            Task::done(message::app(Message::DesktopDialogs(true)))
        } else {
            Task::none()
        };
    ...
    pub fn pop_front(&mut self) -> Option<(DialogPage, Task<Message>)> {
        ...
        let task = if self.pages.is_empty() {
            Task::done(message::app(Message::DesktopDialogs(false)))
        } else {
            Task::none()
        };
        Some((page, task))
                    ...
                    match dialog_page {
                        ...
                        DialogPage::MountError {
                            ...
                            if let Some(mounter) = MOUNTERS.get(&mounter_key) {
                                tasks.push(mounter.mount(item).map(|_| message::none()));
                            }
                        }
            ...
            Message::TabMessage(entity_opt, tab_message) => {
                ...
                for tab_command in tab_commands {
                    match tab_command {
                        ...
                        tab::Command::OpenFile(path) => commands.push(self.open_file(&path)),
